#!/usr/bin/env bash
# tmux setup for use with Browbeat - 1 Controller / 3 ObjectStorage Nodes / 3 Computes
SESSION='browbeat'
...
# Single Pane ObjectStorage Nodes
tmux new-window -t $SESSION:5 -n 'objectstorage'
tmux send-keys "su - stack" C-m
tmux send-keys "cd ${ssh_config_home}" C-m
...
tmux send-keys "su - stack" C-m
tmux send-keys "cd ${ssh_config_home}" C-m
tmux send-keys "ssh -F ssh-config overcloud-objectstorage-1" C-m
tmux send-keys "sudo su -" C-m
tmux split-window -h
tmux select-pane -t 2
tmux send-keys "su - stack" C-m
tmux send-keys "cd ${ssh_config_home}" C-m
tmux send-keys "ssh -F ssh-config overcloud-objectstorage-2" C-m
tmux send-keys "sudo su -" C-m
tmux select-pane -t 0
